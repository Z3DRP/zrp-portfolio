...
export default function Zypher() {
    ...
    const handleSuccessToast = (msg, type) => {
        toast.open(
            <SuccessToast msg={msg} type={type} />
        )
    }
    ...
    const handleZypher = async () => {
        ...
        if (isMissingParams) {
            setHasError(true);
            handleErrorToast(`Error, the following values are required for a zypher: ${[...missingParams]}`, "error");
            return;
        }
        ...
        if (!isValidParams) {
            setHasError(true);
            handleErrorToast(`Error, the following values must be non-negative numbers: ${[...badParams]}`, "error")
            return;
        }
        ...
        await fetchZypher(
            ...
            setHasError(false);
            handleSuccessToast("zypher has been calculated successfully", "success")
    ...
    return (
        <article className="grid grid-cols-4 grid-rows-1 gap-x-4 justify-items-stretch justify-evenly rounded-lg m-4 text-lgreen bg-dgreen dark:bg-dgreen">
            <div id="info-section" className="col-span-2 rounded-md border-solid bg-neutral-800 border-lgreen ">
                <div className='    max-w-3xl p-4 mx-8 mt-8 mb-8'>
                    <h2 className="text-xl mb-4 text-orange-500 font-semibold">Zypher Encryption Tool</h2>
                    <p  className="leading-relaxed text-justify bg-neutral-800 p-2 rounded-md" id="zyphash-description">
                        A tool I made to try to mitigate hash colisions and make it harder for any plain text I hash to be cracked.
                        The Zyphash algorithm is customizable which allows for fine grain control. The idea is a plain text value
                        is first fed through a Zypher then fed through a hash, the iteration count for both of these can be set allowing for
                        multiple rounds of cyphering and hashing. I was going to hash after every shift but I ran into some issues. I ended up taking
                        A page from Bitcoin and just applying the hash N number of times after all shifting is done. It is a one way encryption process so to validate any passwords, etc
            ...
            <div id="zypher-body" className="col-span-2 justify-items-stretch rounded bg-dgreen dark:bg-dgreen w-full p-4 mt-8">
                <ZypherForm
